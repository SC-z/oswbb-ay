...
// Analyzer converts parsed top data into structured analysis.
...
func (a *Analyzer) AnalyzeRange(data *ParsedData, start, end time.Time) (*Analysis, error) {
	if data == nil {
		return nil, fmt.Errorf("top parsed data 不能为空")
	}
	if data.Log == nil {
		return nil, fmt.Errorf("top parsed log 不能为空")
	}
	if len(data.Log.Snapshots) == 0 {
		return nil, fmt.Errorf("top parsed log 没有采样数据")
	}
	...
	return &Analysis{
		Start:      start,
		End:        end,
		DataPoints: len(data.Log.Snapshots),
		Tables: []reportbase.Table{
			legacyoutput.TopRawMetricsTable(legacyoutput.ConvertTopData(data.Log, start, end)),
		},
		Findings: legacyfindings.BuildTopFindingsWithConfig(data.Log, start, end, a.Config),
	}, nil
}
