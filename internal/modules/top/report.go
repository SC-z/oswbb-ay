...
// Reporter builds a top report object without changing existing output.
...
func (r *Reporter) Build(analysis *Analysis) (*reportbase.Report, error) {
	if analysis == nil {
		return nil, fmt.Errorf("top analysis 不能为空")
	}
	...
	return &reportbase.Report{
		Title:       "OSWbb Analyse Report - top",
		Module:      string(modulebase.ModuleTop),
		Summary:     summary,
